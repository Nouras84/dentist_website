...
// a cleaner version without yellow lines
// import React, { useState, useEffect, useCallback } from "react";
// import axios from "axios";
// import { usePatientInfo } from "../../../context/PatientContext";
// import { Modal } from "@mui/material";
...
// perfect but without the date order
// import React, { useState, useEffect, useCallback } from "react";
// import axios from "axios";
// import { usePatientInfo } from "../../../context/PatientContext";
// import { Modal } from "@mui/material";
...
//                     Paciente Id: {patientInfo._id || patientInfo.patientId}
//                   </p>
//                   <p className="hidden-info">Tratamento Id: {treatment._id}</p>
//                   <p>
//                     Data: {formatter.format(new Date(treatment.data))} -
//                     Procedimiento: {treatment.procedimiento} - Dentista:{" "}
//                     {treatment.dentista} - Valor: {treatment.valor} - Nota
...
function TratamentoExecutado() {
  ...
  const addTreatments = async (e) => {
    ...
    if (
      ...
      try {
        // Ensure the date is set to midday to avoid time zone issues
        const treatmentData = {
          ...tratamentoExecutadoData,
          data: new Date(tratamentoExecutadoData.data)
            .toISOString()
            .split("T")[0],
        };
        ...
        await axios.post(
          ...
          {
            tratamentoExecutadoData: treatmentData,
          }
        );
  ...
  const editTratamento = async (e) => {
    ...
    if (
      ...
      try {
        ...
        const editedData = {
          ...patientDataOnEdit,
          data: new Date(patientDataOnEdit.data).toISOString().split("T")[0],
        };
            ...
            fetchedTreatmentsFromDataBase
              ...
              .map((treatment, index) => (
                ...
                >
                  ...
                  <p className="hidden-info">Tratamento Id: {treatment._id}</p>
                  <p>
                    Data: {treatment.data} - Procedimiento:{" "}
                    {treatment.procedimiento} - Dentista: {treatment.dentista} -
                    Valor: {treatment.valor} - Nota Fiscal:{" "}
                    {treatment.notaFiscal} - Forma de Pago:{" "}
                    {treatment.formaDePagamento}
                  </p>
                  <button
                    className="edit-btn"
